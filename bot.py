...
scope = config['scope']
owners = [int(owner_id) for owner_id in config['owners']]
...
async def pull(ctx, server_id, amount=None, country=None):
    ...
    embed.description = f"**Pull to `{ctx.guild.name}`**\n"
...
class Bot(discord.Client):
    ...
    async def on_ready(self):
        ...
        print(f"Bot owners: {owners}")
        print(f"Admin guilds: {admins}")
        await self.tree.sync()
        for guild_id in admins:
            try:
...
@bot.tree.command(name="pull", description="Pull your members to desired server")
async def pull_command(ctx: discord.Interaction, server_id: str, amount: int = None, country: str = None):
    ...
    if guild is None:
        await ctx.response.send_message("Guild not found! Make sure the bot is in it!", ephemeral=True)
        return
    ...
    await ctx.response.send_message(t, ephemeral=True)
    await pull(ctx, server_id, amount, country)


@bot.tree.command(name="getdata", description="Get all member data")
async def getdata(ctx: discord.Interaction):
    if ctx.user.id not in owners:
        await ctx.response.send_message("You are not authorized to use this command.", ephemeral=True)
        return
    
    file = discord.File("data.json")
    await ctx.response.send_message(content="Upload this backup using `/uploaddata`\nRemove unauthorized users using `/usercheck`", file=file, ephemeral=True)


@bot.tree.command(name="uploaddata", description="Upload your data file on the server")
async def uploaddata(ctx: discord.Interaction, file: discord.Attachment):
    if ctx.user.id not in owners:
        await ctx.response.send_message("You are not authorized to use this command.", ephemeral=True)
        return

    if not file.filename.endswith('.json'):
        await ctx.response.send_message("Invalid file format.", ephemeral=True)
        return
    ...
    try:
        ...
        await ctx.response.send_message(f"Data successfully loaded! Total users (unchecked): **{total_users}**\nWarning: If you uploaded users from another app, they will not be valid!\nStart a user check using `/usercheck`")
    except Exception as e:
        await ctx.response.send_message(f"An error occurred while uploading the file: `{e}`", ephemeral=True)


@bot.tree.command(name="usercount", description="Get your verified users count")
async def usercount(ctx: discord.Interaction):
    if ctx.user.id not in owners:
        await ctx.response.send_message("You are not authorized to use this command.", ephemeral=True)
        return
    ...
    await ctx.response.send_message(f"Total authorized users: **{count}**\nStart a user check using `/usercheck` to get accurate user count.", ephemeral=True)


@bot.tree.command(name="verify-embed", description="Create a custom verification embed")
async def verify_embed(ctx: discord.Interaction,
                      channel_id: str,
                      title: str = "Verify",
                      description: str = "Please verify by clicking the button below:",
                      image: str = None,
                      thumbnail: str = None,
    ...
    # استجابة فورية
    await ctx.response.send_message("Creating verification embed...", ephemeral=True)
    ...
    try:
        ...
        if channel is None:
            ...
            await ctx.edit_original_response(content="❌ Channel not found! Make sure the ID is correct.")
            return
        
        print(f"Found channel: {channel.name}")
        login_url_with_state = LOGIN_URL + f"&state={channel.guild.id}"
        print(f"Login URL: {login_url_with_state}")
        ...
        await ctx.edit_original_response(content="✅ Verification embed sent successfully!")
...
async def index():
    ...
    if not code:
        # إضافة header لتجنب رسالة ngrok
        response = redirect(f"{LOGIN_REDIRECT}")
        response.headers['ngrok-skip-browser-warning'] = 'true'
        return response
    
    # إضافة header لتجنب رسالة ngrok
    response = redirect(f"{redirect_uri}/{state}?code={code}")
    response.headers['ngrok-skip-browser-warning'] = 'true'
    return response
...
async def main():
    ...
    # تشغيل الخادم أولاً
    print(f"Starting web server on {server_host}:{server_port}")
    ...
    async with bot:
        try:
            ...
            await asyncio.gather(
                bot.start(token),
                app.run_task(port=int(server_port), host=server_host)
            )
